...
public:
    ...
    bool intersect(const Ray& ray) {
        ...
        Vector3f po = normalize(L);
        float sin2_t = radius2 / dotProduct(L, L);
        float cos_t = dotProduct(ray.direction, po);
        if(cos_t < 0){
            return false;
        }
        return cos_t * cos_t > 1 - sin2_t;
    }
    bool intersect(const Ray& ray, float &tnear, uint32_t &index) const
    {
        ...
        float sin2_t = radius2 / dotProduct(L, L);
        float cos_t = dotProduct(ray.direction, po);
        if(cos_t < 0 || cos_t * cos_t < 1 - sin2_t){
            return false;
        }
        // analytic solution
